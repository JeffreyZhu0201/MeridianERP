...
import { AllocationsView } from '../../allocations/_components/allocations-view';

interface MasterCatalogPageProps {
  searchParams: Promise<{ skuPage?: string; tenantId?: string; status?: string }>;
}

export default async function MasterCatalogPage({ searchParams }: MasterCatalogPageProps) {
  const token = await requireToken();

  const t = await getTranslations('admin.masterCatalog');
    ...
    <AdminShellWithSession>
      <div className="space-y-6">
        ...
        <ListPageFrame title={t('title')} description={t('description')}>
          ...
          <Suspense>
            <ListPagination
              basePath="/inventory/master-catalog"
              total={skuMeta.total}
              page={skuMeta.page}
              limit={skuMeta.limit}
              pageParam="skuPage"
