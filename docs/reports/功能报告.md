...
| 履约方式 | 支付后 | 库存扣减 | 完成条件 |
|----------|--------|----------|----------|
| **自提 PICKUP** | 待核销列表；**不扣**分店库存 | 核销时扣分店默认仓 | `POST .../verify-pickup` → FULFILLED |
| **配送 DELIVERY** | 总部配送队列；**不扣**分店库存 | 发货时扣 `MasterSku` 主仓 | `POST /platform/orders/:id/ship` → FULFILLED |

配送完成时写入 `DeliveryAllocationLedger`（分店虚拟配货成本，计入分店资金报表）。
