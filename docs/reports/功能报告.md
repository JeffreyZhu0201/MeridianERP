...
> **Superseded in part (2026-07-06):** 平台 CRM、要货审核、客户 QR 绑定、零售订单 FULFILLED 计佣等描述已过时。请以 [`docs/PRODUCT.md`](../PRODUCT.md) 为当前产品真相源。
...
| 佣金计提时机 | 分店 **配货单 CONFIRMED**（每家 recruited 分店前 2 笔）；**非**零售订单 FULFILLED |
| 佣金归属 | 分店 `recruitedByDistributorId` → 平台拓店员；`commissionSource=ALLOCATION` |
...
API：`GET /platform/funds/overview?from=&to=`（五项 KPI + 明细端点）、`GET /platform/funds/summary?from=&to=`（兼容）、`GET /merchant/funds/summary?from=&to=`（默认近 30 天）。
...
| S6.5 | ⚠️ 已移除 | 平台 CRM UI/API 已下线；商户 CRM 插件保留 |
...
| 模块 | 端点 |
|------|------|
| 资金 | `GET /platform/funds/overview`；`GET /platform/funds/{inventory-cost,expected-profit,procurement,commissions,net-profit}`；`GET /platform/funds/summary`（兼容） |
| 配货/主 SKU | `GET/POST/PATCH /platform/allocations/master-skus`；`GET/POST /platform/allocations`；`POST .../:id/issue` |
| 分店进货 | `GET /platform/procurement/orders`；`POST .../:id/ship` |
| 分销员 | `GET/POST/PATCH /platform/distributors`；… |
| 分店 | `POST /platform/merchants/:id/approve`；`PATCH .../recruiter` |
| 订单 | `GET /platform/orders?deliveryQueue=true`（总店配送队列）；`GET :id`；`POST :id/ship` |
...
| 要货 | 已移除；改用 `GET/POST /merchant/inventory/procurement` + `GET /platform/procurement/orders` |
...
| 门户 | 路由 | 说明 |
|------|------|------|
| **Admin** | CRM 分组：`/users`、`/merchants`、`/distributors` | 用户、商户、拓店员 |
| | `/inventory/master-catalog` | 主 SKU、分店配货单 |
| | `/allocations` | 总店配送发货队列 |
| | `/procurement` | 分店进货发货 |
| | `/funds` 及 `/funds/*` | 五项资金 KPI + 明细 |
| | `/withdrawals`（含 `?tab=settlements`） | 提现审批 + 佣金结算；`/settlements` 重定向 |
| | `/orders` | 全量订单；配送队列见 `/allocations` |
| **Merchant** | `/orders` | `OrderListFrame`、待核销 tab、`PickupVerifyDialog` |
| | `/allocations` | 待收货确认 |
| | `/funds`, `/inventory/procurement` | 分店资金、总部进货 |
