...
async function loginMerchant(page: Page): Promise<boolean> {
  ...
  if (!res.ok()) {
    test.skip(
      true,
      "演示商户未 seed — 请先运行 pnpm db:migrate && pnpm db:seed",
    );
    return false;
  }
...
test.describe("Phase 3 商户库存（中文 UI）", () => {
  ...
  test("采购历史页加载", async ({ page }) => {
    if (!(await gotoInventoryRoute(page, "/inventory/procurement/history")))
      return;
    await expect(
  ...
  test("库存报表页：指标卡与标签页", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: inv.reports.tabStock }),
    ).toBeVisible();
    await expect(
      page.getByText(inv.reports.totalSkus, { exact: true }).first(),
    ).toBeVisible();
  });
  ...
  test("侧栏库存子导航可访问全部路由", async ({ page }) => {
    const routes = [
      ...
      { href: "/inventory/procurement", label: inv.nav.procurementShop },
      {
        ...
        label: inv.nav.procurementHistory,
      },
    ...
    await expect(page.locator('a[href="/inventory/warehouses"]')).toHaveCount(
      0,
    );
    await expect(
      page.locator('a[href="/inventory/purchase-orders"]'),
    ).toHaveCount(0);
  ...
  test("低库存页：AI 补货建议流式生成", async ({ page }) => {
    if (!(await gotoInventoryRoute(page, "/inventory/alerts"))) return;

    const generateButton = page.getByRole("button", { name: inv.ai.generate });
    await expect(generateButton).toBeVisible();
    await generateButton.click();

    await expect(page.getByText(inv.ai.generating)).toBeVisible();
    await expect(page.getByText(inv.ai.summary)).toBeVisible({
      timeout: 30_000,
    });
    await expect(page.locator("main")).not.toContainText(inv.ai.generating, {
      timeout: 30_000,
    });
  });

  test("去进货页：AI 一键预填进货清单", async ({ page }) => {
    if (!(await gotoInventoryRoute(page, "/inventory/alerts"))) return;
    ...
    await page.getByRole("button", { name: inv.procurement.aiPrefill }).click();
    await expect(
      page.getByText(inv.procurement.aiPrefillLoading),
    ).toBeVisible();
    await expect(page.locator("main")).not.toContainText(
      inv.procurement.aiPrefillLoading,
      {
        timeout: 15_000,
      },
    );
